import {addActor, deleteActor, updateActor, getActor, getActors} from '../actor/interactors/interactor';
import { Request, Response, request} from "express";

export const addActorHttp=(request: Request, response: Response)=>{
    const { body }=request;
    const { actor } =body;
    const result=addActor(actor.id, actor.name, actor.lastname, actor.characterAc, actor.language);
